...
const fetchIncomeCategories = async (userId: string): Promise<IncomeCategory[]> => {
  ...
  return querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as IncomeCategory));
};
...
export default function IncomeCategoriesPage() {
  ...
  const { data: categories = [], isLoading, error } = useQuery<IncomeCategory[], Error>({
  ...
  const addCategoryMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Category Added", description: "New income category has been added." });
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: async (categoryId: string) => {
      ...
      const categoryRef = doc(db, "users", user.uid, "incomeCategories", categoryId);
  ...
  const addButtonDisabled = !canAdd;
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <h1 className="text-2xl sm:text-3xl font-bold font-headline">Income Categories</h1>
        <Dialog open={isDialogOpen} onOpenChange={(open) => {
          ...
          <DialogContent 
            className="w-[90vw] max-w-lg sm:max-w-[425px] max-h-[90vh] overflow-y-auto"
            ...
            {canEditOrDeleteCategory(editingCategory) ? (
              ...
              />
               ...
               <div className="text-center text-muted-foreground py-6">
                  ...
                  <p className="mb-3">
                    {trialLimitReached ? `You have reached the trial limit of ${TRIAL_ITEM_LIMIT} income categories.` : "Your trial or subscription has ended."}
                  </p>
